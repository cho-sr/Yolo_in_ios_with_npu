...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Export best.pt to ExecuTorch for the standalone best_pt iOS app.")
    parser.add_argument(
        "--weights",
        default=str(project_root / "best.pt"),
        help="Path to a YOLO .pt weights file. Defaults to repo-root best.pt",
    )
    parser.add_argument(
        "--imgsz",
        type=int,
    ...
    parser.add_argument(
        ...
        default="cpu_and_ne",
        help="CoreML compute unit for delegated export. 'cpu_and_ne' targets CPU plus Apple Neural Engine.",
    )
    parser.add_argument(
        "--coreml-target",
        default="iOS18",
    ...
    parser.add_argument(
        "--bundle-dir",
        default=str(project_root / "Cho/yolo_best/best_pt/best_pt"),
...
def install_coreml_executorch_exporter(
    ...
    import ultralytics.engine.exporter as ultralytics_exporter
    ...
    ultralytics_exporter.torch2executorch = torch2executorch_coreml
